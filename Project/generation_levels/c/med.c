    #include <stdlib.h>
    #include <omp.h>

void med(float *BaseImage, int maskSize, int dimX, int dimY, float *FinalImage, int padMode) {
    int maskSizediv2 = maskSize / 2;
    int dimX_T = dimX + 2*maskSizediv2;
    int dimY_T = dimY + 2*maskSizediv2;

    float* T = (float*) malloc(dimX_T * dimY_T * sizeof(float));        // T est l'image "padé" de BaseImage
    
    // Construction de T
    for (int i = 0; i < dimX_T; i++) {
        for (int j = 0; j < dimY_T; j++) {
            int orig_i = i - maskSizediv2;
            int orig_j = j - maskSizediv2;
            
            if (orig_i >= 0 && orig_i < dimX && orig_j >= 0 && orig_j < dimY) {
                T[i * dimY_T + j] = BaseImage[orig_i * dimY + orig_j];
            } else {                                    
                if (padMode == 1) {                     // Remplissage avec la derniere valeur vu
                    if (orig_i < 0) orig_i = 0;
                    if (orig_i >= dimX) orig_i = dimX - 1;
                    if (orig_j < 0) orig_j = 0;
                    if (orig_j >= dimY) orig_j = dimY - 1;
                    T[i * dimY_T + j] = BaseImage[orig_i * dimY + orig_j];
                } else {                                // Remplissage par 0
                    T[i * dimY_T + j] = 0.0f;
                }
            }
        }
    }

//-----------------------------------------------------------------------------------------------------

    // construction de FinalImage

    int newDimX = dimX / 2;
    int newDimY = dimY / 2;

    int size = maskSize * maskSize;
    float* tab = (float*) malloc(size * sizeof(float)); // tableau temporaire pour calculer la médiane du des pixels dans le masque appliqué à un pixel


    for (int i=0; i < newDimX; i++) {
        for (int j=0; j < newDimY; j++) {
            int centre_i = 2*i +  1;
            int centre_j = 2*j + 1;
            int idx = 0;

            for (int imar = 0; imar < maskSize; imar++) {
                for (int jmar = 0; jmar < maskSize; jmar++) {
                    tab[idx] = T[(centre_i + imar) * dimY_T + (centre_j + jmar)];
                    idx++;
                }
            }

            // tri par insertion de tab (pour trouver la médiane)
            for (int k = 1; k < size; k++) {
                float cour = tab[k];
                int m = k - 1;
                while (m >= 0 && tab[m] > cour) {
                    tab[m + 1] = tab[m];
                    m = m - 1;
                }
                tab[m + 1] = cour;
            }

            FinalImage[i*newDimY + j] = tab[size / 2];
        }
    }
    free(tab);
}