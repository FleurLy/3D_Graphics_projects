    ...
    void moy(float *A, int maskSize, int dimX, int dimY, int dimZ, float *D, int padMode) 
    {
            ...
            for (int j=0; j < dimY; j++) {
                ...
                for (int imar = -maskSizediv2; imar <= maskSizediv2; imar++) {
                    for (int jmar = -maskSizediv2; jmar <= maskSizediv2; jmar++) {
                        val += T[(i_selon_T + imar) * dimY_T + (j_selon_T + jmar)];
                    }
                }
                D[i*dimY + j] = val / (maskSize*maskSize);
            }
        }
        free(T);
    }